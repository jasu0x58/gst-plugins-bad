	...
	uint32_t* encodeProfileGUIDCount)
{
	...
	return nvenc_api.nvEncGetEncodeProfileGUIDCount(
		encoder, encodeGUID, encodeProfileGUIDCount);
}
	...
	uint32_t* GUIDCount)
{
	...
	return nvenc_api.nvEncGetEncodeProfileGUIDs(
		encoder, encodeGUID, profileGUIDs, guidArraySize, GUIDCount);
}
	...
	NV_ENC_PRESET_CONFIG* presetConfig)
{
	...
	return nvenc_api.nvEncGetEncodePresetConfig(
	...
	int* capsVal)
{
	...
	return nvenc_api.nvEncGetEncodeCaps(
		encoder, encodeGUID, capsParam, capsVal);
}
...
CUcontext gst_nvenc_create_cuda_context(guint device_id)
{
	...
	{
		GST_WARNING(
			"Device with id %d does not exist or does not support NVENC",
			device_id);
		return NULL;
	}
...
typedef NVENCSTATUS(NVENCAPI* NvEntry)(NV_ENCODE_API_FUNCTION_LIST*);

static gboolean plugin_init(GstPlugin* plugin)
{
	GST_DEBUG_CATEGORY_INIT(
		gst_nvenc_debug, "nvenc", 0, "Nvidia NVENC encoder");

	void* nvenclib = dlopen("libnvidia-encode.so.1", RTLD_NOW);
	if (nvenclib == NULL)
	{
		GST_ERROR("Failed to open libnvidia-encode.so!");
		return FALSE;
	}

	NvEntry nvEncodeAPICreateInstance
		= (NvEntry)dlsym(nvenclib, "NvEncodeAPICreateInstance");
	if (nvEncodeAPICreateInstance == NULL)
	{
		GST_ERROR("Failed to load symbol NvEncodeAPICreateInstance!");
		return FALSE;
	}
